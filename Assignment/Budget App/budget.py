class Category:
  ...
  def transfer(self, money_to_transfer, destination_budget):
    if self.check_funds(money_to_transfer):
      self.withdraw(money_to_transfer,
                    "Transfer to " + str(destination_budget.description))
      destination_budget.deposit(money_to_transfer,
...
def create_spend_chart(categories):  #args è una lista di oggetti Category
  ...
  for i in spent_amount:
    ...
    percentage_list.append(int(percentage // 10 *
                               10))  #arrotondo al decimo più vicino
  ...
  for yticks in reversed(range(0, 101,
                               10)):  #creo la barra verticale dell'istogramma
    chart += f"{yticks:>3d}|"
  ...
  # Preparo l'asse x
  x_axes = " " * 4 + "-" * (3 * len(categories)) + "\n"

  # Preparo l'etichetta
  descriptions = [category.description
                  for category in categories]  #['Food', 'Clothing', 'Auto']
  ...
  return (title + chart + x_axes + x_ticks)
